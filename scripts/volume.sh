...
earbuds_cache="$HOME/.config/dwm/cache/earbuds.txt"
...
default_sink=$(pactl info | awk -F': ' '/Default Sink/ {print $2}' | xargs)
    ...
    if pactl get-sink-mute "$default_sink_cache" | grep -q "yes"; then
        echo "$(get_mic_status) ${clcur}󰎄󰅗${cend}"
    else
        ...
        vol_icons=$(switch_string "$vol")
        if [ -n "$earbuds_info" ]; then
            earbuds_battery=${earbuds_info##*|}
            earbuds_icons=$(switch_string "$earbuds_battery")
            echo "$(get_mic_status) ${clbat}󰋌${vol_icons}${ee}󰴚󰆖${earbuds_icons}${ee}${cend}"
        else
            echo "$(get_mic_status) ${clvol}󰎄${vol_icons}${ee}${cend}"
        fi
    fi
